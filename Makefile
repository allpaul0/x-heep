...
# ABI options are any RISC-V ISA string supported by the CPU. Default 'ilp32'
ABI ?= ilp32
...
app: clean-app
	@$(MAKE) -C sw PROJECT=$(PROJECT) TARGET=$(TARGET) LINKER=$(LINKER) LINK_FOLDER=$(LINK_FOLDER) COMPILER=$(COMPILER) COMPILER_PREFIX=$(COMPILER_PREFIX) COMPILER_FLAGS=$(COMPILER_FLAGS) ARCH=$(ARCH) ABI=$(ABI) SOURCE=$(SOURCE) \
	|| { \
