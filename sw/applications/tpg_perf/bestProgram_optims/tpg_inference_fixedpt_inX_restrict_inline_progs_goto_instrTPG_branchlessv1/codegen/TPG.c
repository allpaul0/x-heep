...
/* Helper                                                        */
/* ------------------------------------------------------------ */
static inline int bestProgram(const fixedpt * restrict results, int nb)
{
    int     best = 0;
    fixedpt top  = results[0];

    for (int i = 1; i < nb; i++) {
        fixedpt challenger = results[i];
        /* Branchless: mask is all-1s if challenger >= top, else 0 */
        int32_t mask = -(challenger >= top);   /* 0 or -1 (0xFFFFFFFF) */
        top  = (top  & ~mask) | (challenger & mask);
        best = (best & ~mask) | (i          & mask);
    }
    return best;
}
